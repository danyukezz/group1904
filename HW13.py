...
for price in inner_data['data']:
    ...
    quantity = price.get('rest')
    total_cost += price_products * quantity
pprint(total_cost)
...
for gluten in inner_data['data']:
    if gluten['gluten'] is False:
        ...
        quantity_without_gluten = gluten.get('rest')
        total_cost_without_gluten += price_products_without_gluten * quantity_without_gluten
